...
import services.fedex_generic as fedex_generic
import pytest
import os 
from pathlib import Path
import json

def testFedexGenericDirect(): 
    response=requestLabel('1202_01001274_fedex_label.xml')
    root = ET.fromstring(response)
    label=root.find(".//Label")
    base64Str=label.find("Parts/Image")
    if(len(base64Str.text)<5000):
        pytest.fail("label too short")
    shipmentDetails=root.findall("CompletedShipmentDetail")
    trackingNumber =shipmentDetails[0].find("MasterTrackingId/TrackingNumber").text
    if (len(trackingNumber)<10):
        pytest.fail("tracking_number too short")
    pound_weight=shipmentDetails[0].find("ShipmentRating/ShipmentRateDetails/TotalBillingWeight/Value").text
    if int(float(pound_weight))!=2:
        pytest.fail("Wrong weight.Expected:2, found:"+pound_weight)
    serviceType=shipmentDetails[0].find("ServiceTypeDescription").text
    if (serviceType!='SMART POST'):
        pytest.fail("Wrong service.Expected:Smart Post, found:"+serviceType)
# def testFedexGeneric4Log(): 
#     response=requestLabel(1,'1202_01001274_fedex_label.xml')
#     try:
...
def requestLabel(resouceName):
    myTest=fedex_generic.FedexGeneric()
...
def loadLabelRequest(resourceName):
    ...
    with open(os.path.join(resourcePath,resourceName)) as testResourceFile:
        labelRequest=testResourceFile.read()
    now = datetime.now()
    ship_ts=now.strftime("%Y-%m-%dT%H:%M:%S")
    return labelRequest.replace("*ShipTimestamp*",ship_ts)
